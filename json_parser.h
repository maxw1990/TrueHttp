...
{
    union JsonData {
        JsonData( double d ) : Float( d ){}
        JsonData( long   l ) : Int( l ){}
        JsonData( bool   b ) : Bool( b ){}
        JsonData( string s ) : String( new string( s ) ){}
        JsonData()           : Int( 0 ){}
    ...
    public:
        ...
        JSON(int i): Internal((long)i),Type(Class::Integral){};
        JSON(long i): Internal(i),Type(Class::Integral){};
        JSON(bool b): Internal(b), Type(Class::Boolean){};
        JSON(float f): Internal((double)f), Type(Class::Floating){};
        JSON(string s) : Internal(s), Type(Class::String) {};
        ...
        JSON& operator=(float f){
            SetType( Class::Floating ); Internal.Float = double(f); return *this;
        }
        JSON& operator=(double d){
            SetType( Class::Floating ); Internal.Float = d; return *this;
        }
        ...
        string as_string( int depth = 1, string tab = "  ") const {
            string pad = "";
            ...
            switch( Type ) {
                ...
                case Class::Object: {
                    ...
                    for( auto &p : *Internal.Map ) {
                        if( !skip ) s += ",\n";
                        s += ( pad + "\"" + p.first + "\" : " + p.second.as_string( depth + 1, tab ) );
                        skip = false;
                    }
                    s += ( "\n" + pad.erase( 0, 2 ) + "}" ) ;
                    return s;
